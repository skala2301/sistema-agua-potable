...
class Email extends CI_Model  implements CoreInterface
{
    ...
    public function _render($params = NULL)
    {
        ...
        $this->database_setup(); //if no exist setup into the bdd

        $config['protocol']     =   $this->meta->get_meta_value('smtp_protocol') ;
        $config['smtp_host']    =   $this->meta->get_meta_value("smtp_host");
        $config['smtp_port']    =   $this->meta->get_meta_value("smtp_port") ;
        $config['smtp_timeout'] =   $this->meta->get_meta_value("smtp_timeout");
        $config['smtp_user']    =   $this->meta->get_meta_value("smtp_user");
        $config['smtp_pass']    =   $this->meta->get_meta_value("smtp_pass");

        return $this->load->view('system/email/email_config' ,$config , TRUE);
    }
    ...
    public  function database_setup()
    {
        ...
        {
            if($this->meta->exists_meta_key($key) == 0)
            {
                $this->meta->set_meta($key , $value);
            }
        }
