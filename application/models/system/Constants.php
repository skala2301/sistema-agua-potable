 ...
 * Time: 07:48 PM
 */
class Constants extends CI_Model implements CoreInterface
{
    ...
    public function _render($params = NULL)
    {
        ...
        $const = $this->system->load_config('array')->get_config();
        return $this->load->view("system/constants/index" , ["const" => $const , "count" => count($const) ] , TRUE);
    ...
    public function Save()
    {
        ...
        $request = $_REQUEST['serialize'] ?? NULL;
        if($request == NULL) return json_encode([ "error" => true , "msj" => "no serializable data"]);
        ...
        foreach($request as $req)
        {
            ...
            else if(count($keys) == 2)
            {
                ...
                $object->$value->$value1    = urldecode($item[1]) ?? '';
        ...
        if(count($object) != 0 ){
            $ok = $this->system->set_config($object);
        }



        return json_encode([ "error" => $ok , "msj" => $ok == false ? "Not done !" : "All Done !"]);
